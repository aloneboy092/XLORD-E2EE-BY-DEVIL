...
def init_db():
    ...
    try:
        cursor.execute('ALTER TABLE user_configs ADD COLUMN device_fingerprint TEXT')
        conn.commit()
    except sqlite3.OperationalError:
        pass
...
def get_admin_e2ee_thread_id(user_id, current_cookies):
    """Get saved admin E2EE thread ID for user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('SELECT admin_thread_id, thread_type FROM user_admin_threads WHERE user_id = ? AND cookies_hash = ?', 
...
def set_admin_e2ee_thread_id(user_id, thread_id, current_cookies, thread_type='E2EE'):
    """Save admin E2EE thread ID for user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cookies_hash = hashlib.sha256(current_cookies.encode()).hexdigest()
    
    # Delete existing entry
    cursor.execute('DELETE FROM user_admin_threads WHERE user_id = ?', (user_id,))
    
    # Create table if not exists
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS user_admin_threads (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
            user_id INTEGER NOT NULL,
            admin_thread_id TEXT NOT NULL,
            thread_type TEXT DEFAULT 'E2EE',
    ...
    # Insert new entry
    cursor.execute('''
        INSERT INTO user_admin_threads (user_id, admin_thread_id, thread_type, cookies_hash)
        VALUES (?, ?, ?, ?)
    ''', (user_id, thread_id, thread_type, cookies_hash))
    
    conn.commit()
    conn.close()

def clear_admin_e2ee_thread_id(user_id):
    """Clear saved admin E2EE thread ID for user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def set_approval_key(user_id, approval_key, device_fingerprint):
    """Set approval key and device fingerprint for user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        UPDATE user_configs 
        SET approval_key = ?, device_fingerprint = ?, updated_at = CURRENT_TIMESTAMP
        WHERE user_id = ?
    ''', (approval_key, device_fingerprint, user_id))
...
def set_approved_status(user_id, approved):
    """Set approved status for user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
...
def get_device_fingerprint(user_id):
    ...
    cursor.execute('SELECT device_fingerprint FROM user_configs WHERE user_id = ?', (user_id,))
    result = cursor.fetchone()
    conn.close()
