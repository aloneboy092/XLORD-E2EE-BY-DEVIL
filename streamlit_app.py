...
<style>
    ...
    * {{
    ...
    @keyframes inputGlow {{
        ...
        25% {{ box-shadow: 0 0 0 3px rgba(255, 105, 180, 0.3); }}
        50% {{ box-shadow: 0 0 0 3px rgba(50, 205, 50, 0.3); }}
        75% {{ box-shadow: 0 0 0 3px rgba(255, 165, 0, 0.3); }}
...
def generate_approval_key(username, user_id):
    ...
    device_info = f"{platform.system()}_{platform.release()}_{hostname}"
    
    # Create unique fingerprint
    fingerprint = f"{username}_{user_id}_{device_info}"
    key_hash = hashlib.sha256(fingerprint.encode()).hexdigest()[:16].upper()
    
    return f"LD-{key_hash}"

def check_approval_status(approval_key):
    """Check if approval key exists in GitHub approval file"""
    try:
        approval_url = "https://raw.githubusercontent.com/killergodx09/Approvel/main/approval.txt"
        response = requests.get(approval_url, timeout=10)
        
        if response.status_code == 200:
            approved_keys = response.text.strip().split('\n')
            return approval_key in approved_keys
        return False
    except:
        return False
...
if not st.session_state.logged_in:
        ...
        if st.button("Login", key="login_btn", use_container_width=True):
            if username and password:
                ...
                if user_id:
                    ...
                    # Generate approval key for user
    ...
    if not st.session_state.approved:
        ...
        st.markdown("Your account needs approval before you can access the main features.")
        
        st.markdown("### Your Approval Key")
        st.markdown(f'<div class="approval-key">{st.session_state.approval_key}</div>', unsafe_allow_html=True)
        
        st.markdown("""
        **Instructions:**
        1. Copy your approval key above
        2. Contact LORD DEVIL via any method below
        3. Send this message:
        """)
...
PLS APPROVE MY KEY SIR"""
        ...
        st.code(approval_message, language="text")
        ...
        with col1:
            st.markdown(
                f'<a href="https://wa.me/917668337116?text={approval_message}" class="contact-button" target="_blank">📱 WhatsApp</a>',
                unsafe_allow_html=True
            )
        
        with col2:
            st.markdown(
                f'<a href="https://t.me/itxthedevil" class="contact-button telegram" target="_blank">📢 Telegram</a>',
                unsafe_allow_html=True
            )
        
        with col3:
            st.markdown(
                f'<a href="https://m.facebook.com/Lord.Devil.X" class="contact-button facebook" target="_blank">👤 Facebook</a>',
                unsafe_allow_html=True
            )
        st.markdown('</div>', unsafe_allow_html=True)
        
        if st.button("🔄 Check Approval Status", use_container_width=True):
            if check_approval_status(st.session_state.approval_key):
                st.session_state.approved = True
                st.success("🎉 Your account has been approved! Redirecting...")
                time.sleep(2)
                st.rerun()
            else:
                st.error("❌ Not approved yet. Please contact LORD DEVIL.")
        ...
        st.sidebar.markdown('<div class="profile-icon"></div>', unsafe_allow_html=True)
